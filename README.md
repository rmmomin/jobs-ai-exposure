...
After running the comparison scripts, generate a combined measure summary table:

```bash
uv run python scripts/summarize_comparison_measures.py
```

This writes `data/exports/comparisons/tables/measure_comparison_summary.csv`, combining occupation and industry comparison measures with source workbook/sheet, grain, loaded rows, and maximum overlap against internal variants.

Current measure coverage:

| Level | Measure | Source | Grain |
|---|---|---|---|
| Occupation | Felten base AIOE | `AIOE_DataAppendix.xlsx` (`Appendix A`) | SOC |
| Occupation | Felten language-modeling AIOE | `Language_Modeling_AIOE_and_AIIE.xlsx` (`LM AIOE`) | SOC |
| Occupation | Felten image-generation AIOE | `Image_Generation_AIOE_and_AIIE.xlsx` (`IG AIOE`) | SOC |
| Occupation | OpenAI GPTs-are-GPTs | `openai_occ_level.csv` | SOC |
| Occupation | Microsoft AI applicability | `microsoft_ai_applicability_scores.csv` | SOC |
| Occupation | Eisfeldt occupation GenAI exposure | `genaiexp_estz_occscores.csv` | SOC |
| Occupation | Webb exposure | `exposure_by_soc4.xlsx` (`exposure_by_soc4`) | SOC4 |
| Industry | Felten base AIIE | `AIOE_DataAppendix.xlsx` (`Appendix B`) | NAICS-4 |
| Industry | Felten language-modeling AIIE | `Language_Modeling_AIOE_and_AIIE.xlsx` (`LM AIIE`) | NAICS-4 |
| Industry | Felten image-generation AIIE | `Image_Generation_AIOE_and_AIIE.xlsx` (`IG AIIE`) | NAICS-4 |
