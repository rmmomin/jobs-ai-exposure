...
def add_standardized_columns(df, value_col: str = "value"):
    """Attach numeric value, z-score, and percentile columns to a DataFrame.

    The caller is expected to pass a pandas DataFrame and a column name that
    contains the source metric values. This helper keeps transformation logic
    consistent between occupation and industry comparison scripts.
    """

    # Local import to keep the helper module lightweight at import time.
    import pandas as pd

    out = df.copy()
    out[value_col] = pd.to_numeric(out[value_col], errors="coerce")
    out = out.dropna(subset=[value_col]).copy()
    values = out[value_col].tolist()
    out[f"{value_col}_z"] = zscore(values)
    out[f"{value_col}_pct"] = percentile(values)
    return out
