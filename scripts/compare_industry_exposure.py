...
This script is intentionally NAICS-4 focused to align the canonical
`industry_exposure_4digit.csv` output with Felten AIIE workbook sheets.
"""
...
from comparison_utils import add_standardized_columns, ensure_dir, naics4, pearson, spearman
...
def load_internal_variants(output_root: Path) -> tuple[dict[str, pd.DataFrame], list[str]]:
    """Load canonical and optional custom 4-digit industry exposure variants."""
    variants: dict[str, pd.DataFrame] = {}
    issues: list[str] = []

    current = pd.read_csv(INDUSTRY_EXPOSURE_4DIGIT_CSV)
    current["naics4"] = current["naics_code"].map(naics4)
    current["value"] = current["weighted_exposure"]
    current = current.dropna(subset=["naics4"]).copy()
    current = add_standardized_columns(current, "value")
    ...
    for variant, path in candidate_files.items():
        ...
        try:
            ...
            df = df.dropna(subset=["naics4"]).copy()
            df = add_standardized_columns(df, "value")
            df["variant"] = variant
            variants[variant] = df
...
def _clean_aiie_sheet(df: pd.DataFrame, source_file: str, sheet_name: str) -> pd.DataFrame | None:
    """Normalize a Felten AIIE worksheet to NAICS-4 and standardized value columns."""
    naics_col = None
    metric_col = None
    ...
    out = out.dropna(subset=["naics4"]).copy()
...
def load_external_aiie_metrics() -> tuple[dict[str, pd.DataFrame], list[str]]:
    """Load the three external Felten industry AIIE metrics required by README_codex."""
