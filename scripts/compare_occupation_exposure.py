...
from comparison_utils import ensure_dir, norm_soc, pearson, percentile, soc4, spearman, zscore
...
def normalize_internal_variant(label: str, df: pd.DataFrame, lookup: pd.DataFrame) -> pd.DataFrame:
    ...
    base["value"] = _as_numeric(base["exposure"])
    base = base.dropna(subset=["value"]).copy()
    values = base["value"].tolist()
    base["value_z"] = zscore(values)
    base["value_pct"] = percentile(values)
    base["variant"] = label
...
def _clean_metric_df(df: pd.DataFrame, source_file: str, sheet_name: str | None) -> pd.DataFrame | None:
    ...
    out["value"] = _as_numeric(df[value_col])
    ...
    out = out.dropna(subset=["value", "join_code"]).copy()
    if out.empty:
        return None

    values = out["value"].tolist()
    out["value_z"] = zscore(values)
    out["value_pct"] = percentile(values)
    return out
